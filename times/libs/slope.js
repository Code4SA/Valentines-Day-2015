!function(){
	...
	function visualize(data){
		var vis ={};
		function calculatePosition(a, s, e, b, m){ //, total){

			var total= d3.sum(a);
			var sum=0, neededHeight=0, leftoverHeight= e-s-2*b*a.length;
			var ret =[];
      //var all = [197,197,197,197,197,107,107,107,107,107,107];
			a.forEach(
				function(d, i ){
					var v={};
					v.percent = (total == 0 ? 0 : d); ///all[i]);
					v.value=d;
			...
			ret.forEach(
				function(d, i){
					...
					//d.percent = (total == 0 ? 0 : d.value/all[i]);
					sum+=2*b+d.height;
				}
			);
			return ret;
	...
	function drawPart(data, id, p){ // is is a second column
	...
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2]) //title
			.style("font-size","17px").attr("x",108).attr("y",-25).style("text-anchor","middle")
		...
		[0,1].forEach(function(d){
			...
			h.append("line").attr("x1",c1[d]-10).attr("y1", 2) //-30 is margin to the left
				...
				.style("stroke-width","2").style("shape-rendering","crispEdges")
				.style("stroke-dasharray", ("4, 9"));
		});
	}
  ...
  // MAIN FUNCTION
	bP.draw = function(data, svg){
		data.forEach(function(biP,s){
			...
			drawPart(visData, biP.id, 0); //draws the bar on the left
			drawPart(visData, biP.id, 1); //draws the bar on the right
			//drawPart(visData, biP.id, 2); // added third bar
			drawEdges(visData, biP.id); //draws the joining points
	...
	bP.selectSegment = function(data, m, s){
		data.forEach(function(k){
		  ...
		  // modify bar and labels upon selection
			selectedBar.select(".mainrect").style("stroke-opacity",5).style("stroke","white").style("fill", "green");
			//selectedBar.select(".barlabel").style('font-size', '22px');
			selectedBar.select(".barlabel").style('font-weight','bold').style('font-size', '11px');
			selectedBar.select(".barvalue").style('font-weight','bold').style('color','red');
